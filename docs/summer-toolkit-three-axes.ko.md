...
V_PD는 우리 손으로 구현했다 (C3b 완료, sv-4 `8a52ca1`).
(Paul의 그림에서 읽은 값이 아니라 우리 계산으로 확인한 것이다.)

그리고 turnstile flux(sv-5, `add102b`)가 가설을 직접 측정했다:
ΔW는 m=4 → m=36으로 두 자릿수에 걸쳐 단조감소하며
(6.46×10⁻³ → 8.41×10⁻⁵), 수송 지표와 정렬한다
— corr(log ΔW, V_PD) = +0.92, corr(log ΔW, ΔT) = −0.98.

**★ 가장 많이 깨진 자기장이 가장 강한 cantori를 가진다. 가설은 supported다.**

**★ 단, supported이지 proven이 아니다.** 궤도 추적은 q ≤ 5까지만 된다
(q = 8에서 residue ~200 — q → ∞ cantorus 극한에는 미도달).
q|m 공명 제약 때문에 m=20은 convergent 2/3, 나머지는 3/5다.
순서는 이 선택들에 견고하다.

**★ 남은 열린 문제는 q → ∞ 극한이다. Paul이 2022년에 남긴 비교 자체는 끝났다.**
