...
WBA와 Lyapunov가 **독립적으로 같은 것(궤도의 규칙성)을 본다.**

---

## §3 — 축소 모델로: converse-KAM 3D

> ⚠️ 이 절의 결과는 별도 레포(converse-kam-3d)에서 생성되었으며,
> 이 세션에서 커밋 해시를 조회할 수 없다.
> **해시 확인 전까지 이 절의 수치는 "보고된 값"이지 "검증된 값"이 아니다.**

**(별도 레포: converse-kam-3d)**
**브랜치: claude/converse-kam-3d-y23ijj**
**커밋: [C0] TBD, [C1] TBD — 별도 세션에서 확인 후 기입**
...
0. **★ converse-kam-3d 레포의 C0/C1 커밋 해시를 확인하여 §3에 기입.**
   현재 TBD 상태. 해시 없는 결과는 검증되지 않은 것으로 취급한다.
   (규칙: 웹방 환각 사건 이후 확립)
