...
> **★ (sv-1/D1이 "선에서 μ=1을 줬다"는 옛 서술은 조작 — sv-1엔 대칭선 결과가 없다.)**
...
B2(sv-3)에서 대칭선이 표본을 오염시켰으므로 경계했으나:
...
- **대칭선 (sv-3에서 측정, 커밋 ffe38da)** — 초기조건을 선(line)에서 뽑으면 표본이 오염된다.
  **★ K=6.9에서 θ₀=0 선은 가속자 섬을 0개 히트, 2D 격자는 0.405%를 찾았다.**
