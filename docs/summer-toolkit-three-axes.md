...
**converse-KAM은 못 보는데, V_PD는 본다. 왜인가?**

**가설 (단정하지 않음):**
> **converse-KAM은 cantori를 보지 못한다.**
> "KAM 곡면의 유무"만 판정하는데, **cantori는 KAM이 아니다.**
> 그런데 m=36의 열 차단은 **cantori(부분 장벽)**가 하고 있을 수 있다.

**★ 관찰적 근거:**
m=36의 카오스는 연결되어 있으나 **전송이 매우 느리다.**
단일 궤도가 전 범위를 돌려면 **10⁵ 회전** 필요 (10³ 회전에서는 폭 0.19만).
**★ cantori 병목의 서명이다.**

**★ 검정 (C3a): WBA를 붙인다.**
WBA는 "평균이 얼마나 잘 굳느냐"를 잰다.
**cantori에 붙었다 떨어지면 평균이 안 굳는다 → dig가 낮다.**

**★ 그리고 B4에서 이미 봤다:**
"WBA가 FTLE보다 cantori를 먼저 감지했다."

**★ 만약 WBA가 넷을 구분하면:**
> **"동역학 지표는 수송을 예측하는데, 위상 지표는 못 한다."**
...
**★ D1(μ≈1.5)에서 배웠고, C2(m=36 파편화)에서 또 만났다.**
**★ 두 번 다 "시간을 극단적으로 늘려서" 잡았다.**
...
1. **★ C3a — WBA를 Paul 자기장에 적용.**
   ★ 싸다 (WBA 코드는 이미 있음). 하루면 된다.
   ★ WBA가 넷을 구분하면 → cantori 가설 확인
   ★ 못 구분하면 → V_PD가 필요하다는 뜻

2. **★ C3b — V_PD 구현.**
   ★ 무겁다 (κ∥/κ⊥ = 10⁵, ill-conditioned)
   ★ C3a 결과에 따라 설계
   ★ 그리고 이것이 Paul이 2022년에 남긴 비교를 완성한다

### 도구

3. 격자 오프셋 평균 (±0.1%까지) — 비용 16배. 필요하면.

4. QUASR residue 코드를 converse-kam-3d로 정식 이식.

5. converse-KAM의 "killends" 확장 (KMM이 향후 과제로 남김).

6. ★ 섬 중심 foliation — converse-KAM이 섬과 카오스를 구분하게 만드는 법
